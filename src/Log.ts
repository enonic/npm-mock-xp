	...
	static createLogger({
		...
		return {
			...
			error: loglevel !== 'silent'
				? (format: string, ...s: unknown[]): void => {
					if (s.length) {
						console.debug(brightRed(`ERROR ${format}`), ...Log.colorize(s));
					} else {
						console.debug(brightRed(`ERROR ${format}`));
					}
				}
				: (_format: string, ..._s: unknown[]): void => {},
			info: loglevel !== 'silent' && loglevel !== 'error' && loglevel !== 'warn'
				? (format: string, ...s: unknown[]): void => {
					if (s.length) {
						console.debug(white(`INFO  ${format}`), ...Log.colorize(s));
					} else {
						console.debug(white(`INFO  ${format}`));
					}
				}
				: (_format: string, ..._s: unknown[]): void => {},
			warning: loglevel !== 'silent' && loglevel !== 'error'
				? (format: string, ...s: unknown[]): void => {
					if (s.length) {
						console.debug(brightYellow(`WARN  ${format}`), ...Log.colorize(s));
					} else {
						console.debug(brightYellow(`WARN  ${format}`));
					}
				}
				: (_format: string, ..._s: unknown[]): void => {},
		};
