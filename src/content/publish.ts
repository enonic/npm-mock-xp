...
const LOG_PREFIX = 'mock-xp: lib-content.publish():';


export function publish(params: PublishContentParams): PublishContentResult {
	if (!globalThis._javaBridge) {
		throw new Error(`${LOG_PREFIX} In order to use the lib-content.publish() mock, an instance of JavaBridge must be created and assigned to globalThis._javaBridge`);
	}
	...
	return contentConnection.publish(params);
}
